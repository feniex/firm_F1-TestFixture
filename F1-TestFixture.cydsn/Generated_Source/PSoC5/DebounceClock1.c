...
/*******************************************************************************
* Function Name: DebounceClock1_Start
********************************************************************************
*
* Summary:
...
void DebounceClock1_Start(void) 
{
    ...
    DebounceClock1_CLKEN |= DebounceClock1_CLKEN_MASK;
	DebounceClock1_CLKSTBY |= DebounceClock1_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: DebounceClock1_Stop
********************************************************************************
*
* Summary:
...
void DebounceClock1_Stop(void) 
{
    ...
    DebounceClock1_CLKEN &= (uint8)(~DebounceClock1_CLKEN_MASK);
	DebounceClock1_CLKSTBY &= (uint8)(~DebounceClock1_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: DebounceClock1_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((DebounceClock1_CLKEN & DebounceClock1_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = DebounceClock1_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = DebounceClock1_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* DebounceClock1__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(DebounceClock1_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(DebounceClock1_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: DebounceClock1_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: DebounceClock1_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = DebounceClock1_GetSourceRegister();
    uint16 oldDivider = DebounceClock1_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = DebounceClock1_CLKEN & DebounceClock1_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(DebounceClock1_DIV_PTR, clkDivider);
                DebounceClock1_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                DebounceClock1_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(DebounceClock1_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((DebounceClock1_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: DebounceClock1_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(DebounceClock1_DIV_PTR);
}


/*******************************************************************************
* Function Name: DebounceClock1_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    DebounceClock1_MOD_SRC |= modeBitMask & (uint8)DebounceClock1_MODE_MASK;
}


/*******************************************************************************
* Function Name: DebounceClock1_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    DebounceClock1_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(DebounceClock1_MODE_MASK));
}


/*******************************************************************************
* Function Name: DebounceClock1_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return DebounceClock1_MOD_SRC & (uint8)(DebounceClock1_MODE_MASK);
}


/*******************************************************************************
* Function Name: DebounceClock1_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = DebounceClock1_GetDividerRegister();
    uint8 oldSrc = DebounceClock1_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        DebounceClock1_MOD_SRC |= CYCLK_SSS;
        DebounceClock1_MOD_SRC =
            (DebounceClock1_MOD_SRC & (uint8)(~DebounceClock1_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: DebounceClock1_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return DebounceClock1_MOD_SRC & DebounceClock1_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: DebounceClock1_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    DebounceClock1_PHASE = clkPhase & DebounceClock1_PHASE_MASK;
}


/*******************************************************************************
* Function Name: DebounceClock1_GetPhase
********************************************************************************
*
* Summary:
...
{
    return DebounceClock1_PHASE & DebounceClock1_PHASE_MASK;
}
