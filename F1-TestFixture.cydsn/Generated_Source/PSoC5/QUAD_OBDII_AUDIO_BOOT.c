/*******************************************************************************
* File Name: QUAD_OBDII_AUDIO_BOOT.c
* Version 2.50
*
* Description:
...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_QUAD_OBDII_AUDIO) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)))


/*******************************************************************************
* Function Name: QUAD_OBDII_AUDIO_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: QUAD_OBDII_AUDIO_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: QUAD_OBDII_AUDIO_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: QUAD_OBDII_AUDIO_CyBtldrCommWrite
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    while(bufIndex < size)
    {
        QUAD_OBDII_AUDIO_PutChar(pData[bufIndex]);
        bufIndex++;
    }
...
/*******************************************************************************
* Function Name: QUAD_OBDII_AUDIO_CyBtldrCommRead
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != QUAD_OBDII_AUDIO_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = QUAD_OBDII_AUDIO_GetRxBufferSize();
                CyDelay(QUAD_OBDII_AUDIO_BYTE2BYTE_TIME_OUT);
            }
            while(QUAD_OBDII_AUDIO_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  QUAD_OBDII_AUDIO_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(QUAD_OBDII_AUDIO_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(QUAD_OBDII_AUDIO_GetRxBufferSize() > 0u)
    {
        tempCount = QUAD_OBDII_AUDIO_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = QUAD_OBDII_AUDIO_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional QUAD_OBDII_AUDIO_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != QUAD_OBDII_AUDIO_PACKET_EOP)
            {
                CyDelay(QUAD_OBDII_AUDIO_WAIT_EOP_DELAY);
            }
        }
