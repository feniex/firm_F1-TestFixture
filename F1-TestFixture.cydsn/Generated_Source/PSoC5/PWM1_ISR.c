...
void PWM1_ISR_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)PWM1_ISR__INTC_NUMBER] = address;
}
...
cyisraddress PWM1_ISR_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)PWM1_ISR__INTC_NUMBER];
}
...
{
    *PWM1_ISR_INTC_PRIOR = priority << 5;
}
...
uint8 PWM1_ISR_GetPriority(void)
{
    ...
    priority = *PWM1_ISR_INTC_PRIOR >> 5;
