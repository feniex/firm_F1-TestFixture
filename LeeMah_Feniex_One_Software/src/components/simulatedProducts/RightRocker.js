...
export class LightStickRightRocker extends Component {
    ...
    static getDerivedStateFromProps(props, current_state) {
        var startOfRightRocker = 94;
        tempBuffer = JSON.parse(props.frameBufferProps);
        //console.log(tempBuffer);
        for(var i = startOfRightRocker; i<8+startOfRightRocker; i++)
        {
            //console.log(tempBuffer[i]);
            if((tempBuffer[i]&0x03) === 0x03)
            {
                //console.log("Blue");
                current_state.frontStyles[i-startOfRightRocker] = barBlue100;
            }
            else if((tempBuffer[i]&0x03) === 0x02)
            {
                //console.log("Blue 50");
                current_state.frontStyles[i-startOfRightRocker] = barBlue50;
            }
            else if((tempBuffer[i]&0x03) === 0x01)
            {
                //console.log("Blue 10");
                current_state.frontStyles[i-startOfRightRocker] = barBlue10;
            }
            else if((tempBuffer[i]&0x0C) === 0x0C)
            {
                //console.log("Blue");
                current_state.frontStyles[i-startOfRightRocker] = barRed100;
            }
            else if((tempBuffer[i]&0x0C) === 0x08)
            {
                //console.log("Blue");
                current_state.frontStyles[i-startOfRightRocker] = barRed50;
            }
            else if((tempBuffer[i]&0x0C) === 0x04)
            {
                //console.log("Blue");
                current_state.frontStyles[i-startOfRightRocker] = barRed10;
            }
            else if((tempBuffer[i]&0x30) === 0x30)
            {
                current_state.frontStyles[i-startOfRightRocker] = barAmber100;
            }
            else if((tempBuffer[i]&0x30) === 0x20)
            {
                current_state.frontStyles[i-startOfRightRocker] = barAmber50;
            }
            else if((tempBuffer[i]&0x30) === 0x10)
            {
                current_state.frontStyles[i-startOfRightRocker] = barAmber10;
            }
            else if((tempBuffer[i]&0xC0) === 0xC0)
            {
                current_state.frontStyles[i-startOfRightRocker] = barWhite100;
            }
            else if((tempBuffer[i]&0xC0) === 0x80)
            {
                current_state.frontStyles[i-startOfRightRocker] = barWhite50;
            }
            else if((tempBuffer[i]&0xC0) === 0x40)
            {
                current_state.frontStyles[i-startOfRightRocker] = barWhite10;
            }
            else
            {
                current_state.frontStyles[i-startOfRightRocker] = visiblityTrue;
            }
        }
    ...
    render() {
        return (
            <div className="lightStickRightRocker">
                <div className="lsRRocker_m1" style={this.state.frontStyles[0]}>
                </div>
                <div className="lsRRocker_m2" style={this.state.frontStyles[1]}>
                </div>
                <div className="lsRRocker_m3" style={this.state.frontStyles[2]}>
                </div>
                <div className="lsRRocker_m4" style={this.state.frontStyles[3]}>
                </div>
                <div className="lsRRocker_m5" style={this.state.frontStyles[4]}>
                </div>
                <div className="lsRRocker_m6" style={this.state.frontStyles[5]}>
                </div>
                <div className="lsRRocker_m7" style={this.state.frontStyles[6]}>
                </div>
                <div className="lsRRocker_m8" style={this.state.frontStyles[7]}>
                </div>
            </div>
        );
    }
